...
function M.extend_tooltip(tooltip, link, quantity)
    ...
    if item_info then
        ...
        if getn(distribution) > 0 then
            if settings.disenchant_distribution then
                tooltip:AddLine('Se désenchante en :', aux.color.tooltip.disenchant.distribution())
            ...
            if settings.disenchant_value then
                ...
                tooltip:AddLine('Désenchantement : ' .. (disenchant_value and money.to_string2(disenchant_value) or UNKNOWN), aux.color.tooltip.disenchant.value())
            end
        end
    end
    if settings.merchant_buy then
        ...
        if price then
            tooltip:AddLine('Achat marchand ' .. (limited and '(limité) : ' or ': ') .. money.to_string2(price * quantity), aux.color.tooltip.merchant())
        end
    end
    if settings.merchant_sell then
        ...
        if price ~= 0 then
            tooltip:AddLine('Vendeur : ' .. (price and money.to_string2(price * quantity) or UNKNOWN), aux.color.tooltip.merchant())
        end
    end
    ...
    if auctionable then
        if settings.value then
            tooltip:AddLine('Valeur : ' .. (value and money.to_string2(value * quantity) or UNKNOWN), aux.color.tooltip.value())
        end
        if settings.daily  then
            local market_value = history.market_value(item_key)
            tooltip:AddLine("Aujourd'hui : " .. (market_value and money.to_string2(market_value * quantity) .. ' (' .. gui.percentage_historical(aux.round(market_value / value * 100)) .. ')' or UNKNOWN), aux.color.tooltip.value())
        end
    end
