...
function M.set_shopping_tooltip(slot)
    ...
    if tooltips[1] then
        tinsert(tooltips[1], 1, temp-O('left_text', 'Actuellement équipé', 'left_color', temp-A(.5, .5, .5)))
    ...
    if tooltips[2] then
        tinsert(tooltips[2], 1, temp-O('left_text', 'Actuellement équipé', 'left_color', temp-A(.5, .5, .5)))
