...
function M.execute(resume, real_time)
	...
	elseif real_time then
		if getn(queries) > 1 then
			print('Erreur : le mode temps réel ne prend pas en charge les recherches multiples')
			return
		elseif queries[1].blizzard_query.first_page or queries[1].blizzard_query.last_page then
			print('Erreur : le mode temps réel ne prend pas en charge les plages de pages')
			return
		end
	end
